...
export default function EditPurchaseOrder() {
  ...
  const saveOrder = async (status: PurchaseOrderStatus) => {
    ...
    try {
      ...
      if (response.ok) {
        toast({
          title: "Success",
          description: `Purchase order ${status === PurchaseOrderStatus.DRAFT ? 'saved as draft' : 'updated'} successfully`
        });
        router.push('/purchase-orders');
      } else {
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-gray-600 mt-2">
                Modify purchase order {purchaseOrder.orderNumber}
              </p>
            </div>
          </div>
          <div className="flex space-x-3">
            ...
            <Button variant="outline" onClick={() => saveOrder(PurchaseOrderStatus.DRAFT)} disabled={isLoading}>
              <Save className="mr-2 h-4 w-4" />
              Save Draft
            </Button>
            <Button onClick={() => saveOrder(purchaseOrder.status)} disabled={isLoading}>
